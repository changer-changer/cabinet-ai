...
**思维模型库**（借鉴女娲skill的思维模型蒸馏方式）：

EliteAdvisor 借鉴女娲skill的深度调研方法，对每个思维模型进行完整的**操作系统级蒸馏**——包含核心心智模型、决策启发式、表达DNA和经典错误。当你问"如果是Musk会怎么做"，EliteAdvisor不是给你一个标签，而是真正以Musk的身份思考：用渐近极限法拆解成本、用五步算法质疑需求、用"物理定律是唯一硬约束"挑战假设，甚至用他的极简宣言体给出判断。

每个模式都是一个完整的思维操作系统，根据问题自动切换：
- **技术/物理极限** → Musk模式（渐近极限法、五步算法、快速迭代）
- **商业/客户价值** → Bezos模式（逆向工作法、遗憾最小化、Day 1心态）
- **产品/用户体验** → Jobs模式（聚焦即说不、端到端控制、死亡过滤器）
- **投资/风险** → Buffett + Munger模式（安全边际、能力圈、逆向思考）
- **竞争/创业** → Thiel模式（垄断思维、从0到1、秘密发现）
- **AI/工程** → Karpathy模式（Software X.0、构建即理解、March of Nines）
- **研究/智能本质** → Ilya模式（压缩即理解、安全-能力纠缠、研究审美）
- **内容/注意力** → MrBeast模式（CTR×AVD、零无聊时刻、阶梯递进）
- **谈判/权力** → Trump模式（万物皆交易、不可预测性、零和赢）
- **创业/迭代** → Paul Graham模式（Writing=Thinking、迭代发现、超线性回报）
- **信息效率/组织** → 张一鸣模式（延迟满足、高维投影、Context not Control）
